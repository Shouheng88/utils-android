...
public final class ClipboardUtils {
    ...
    public static void copyText(final CharSequence text) {
        ClipboardManager cm = (ClipboardManager) UtilsApp.getApp().getSystemService(Context.CLIPBOARD_SERVICE);
        // noinspection ConstantConditions
        cm.setPrimaryClip(ClipData.newPlainText(UtilsApp.getApp().getPackageName(), text));
    }
    ...
    public static CharSequence getText() {
        ...
        if (clip != null && clip.getItemCount() > 0) {
            CharSequence text = clip.getItemAt(0).coerceToText(UtilsApp.getApp());
            if (text != null) {
                return text;
            }
        }
